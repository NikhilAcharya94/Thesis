...
ENTITY = Namespace("http://www.semanticweb.org/nacharya/ontologies/2020/11/Itunes-Ontology#")
g = rdflib.Graph()
...
def populate_rdf(Entity_df):
    Itunes_ns_string="http://www.semanticweb.org/nacharya/ontologies/2020/11/Itunes-Ontology#"
    Entity_ns_string="http://www.semanticweb.org/nacharya/ontologies/2020/11/Itunes-Ontology#"
    for index, row in Entity_df.iterrows():
        entity_id=Entity_ns_string+str(int(row["id"]))+"L"
        album_entity=Entity_ns_string+ validate_uri(row["Album_Name"])
        artist_entity=Entity_ns_string+validate_uri(row["Artist_Name"])
        Time_entity=Entity_ns_string+str(int(row["id"]))+"L"+"Time"
        Price_entity=Entity_ns_string+str(int(row["id"]))+"L"+"Price"
        Genre_entity=Entity_ns_string+str(int(row["id"]))+"L"+"Genre"
        #Class Definition
...
def main(file):
    g.parse ('output_file.ttl', format='ttl')
    itunes_data=pd.read_csv('train_conflicts5.csv')
    columns_list=list(itunes_data.columns[0:10])
